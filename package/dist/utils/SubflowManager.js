...
export class SubflowManager {
    constructor(connection, schemaManager) {
        this.connection = connection;
        this.schemaManager = schemaManager;
        this.subflowCache = new Map();
    }
    async analyzeSubflow(subflowName) {
    ...
    async getSubflowMetadata(subflowName) {
    ...
    async analyzeSubflowMetadata(metadata) {
        let dmlOperations = 0;
        let soqlQueries = 0;
        let complexity = 0;
        ...
        if (metadata.recordLookups)
            soqlQueries += this.countElements(metadata.recordLookups);
    ...
    shouldBulkifySubflow(dmlOps, soqlQueries, complexity, metadata) {
        // Always bulkify if has DML or SOQL
        if (dmlOps > 0 || soqlQueries > 0)
            return true;
        // Bulkify if complex
        if (complexity > 5)
            return true;
    ...
    getBulkificationReason(dmlOps, soqlQueries, complexity, metadata) {
        const reasons = [];
        if (dmlOps > 0)
            reasons.push(`Contains ${dmlOps} DML operation(s)`);
        if (soqlQueries > 0)
            reasons.push(`Contains ${soqlQueries} SOQL queries`);
        if (complexity > 5)
...
//# sourceMappingURL=SubflowManager.js.map